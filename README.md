...
**Short links.** Google's Share button copies the place name next to a short maps.app.goo.gl link. Paste both together and the name is read from the text. A short link pasted on its own is expanded by a small helper, `resolver.gs`, a Google Apps Script that follows the redirect; its address is built into the board.
...
The Google Maps browser key is built into the board and restricted to the site's address in Google Cloud, where the Maps JavaScript, Places (New), Directions and Geocoding APIs must be enabled and billing linked. If Google refuses the key, the status line says why in plain words.
